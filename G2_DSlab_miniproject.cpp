...
public:
    ...
    {   for(auto i=pins_list.begin();i!=pins_list.end();i++)
        {   if(i->district==this->district_name)
                {pin.push_back(*i);no_of_center+=i->center.size();}
        }
...
public:
    ...
    states(string state)
    {   s_name=state;
        add_district();no_of_districts=district.size();
    }
    states(){};
...
if(value==1)
{setfillstyle(1,RED); setcolor(YELLOW);}
switch(n)
{
case 0:fillellipse(225,280,3,3);outtextxy(229,270,(char*)"Maharashtra");    //Mumbai
	break;
case 1:fillellipse(229,233,3,3);outtextxy(232,223,(char*)"Gujrat"); //Ahmedabad
	break;
case 2:fillellipse(255,344,3,3);outtextxy(259,334,(char*)"Goa");       //Goa
	break;
case 3:fillellipse(288,171,3,3);outtextxy(292,161,(char*)"Rajasthan");    //Jaipur
	break;
case 4:fillellipse(304,396,3,3);outtextxy(308,385,(char*)"Karnataka"); //Bengaluru
	break;
case 5:fillellipse(308,129,3,3);outtextxy(312,119,(char*)"Delhi");     //Delhi
	break;
case 6:fillellipse(319,319,3,3);outtextxy(323,309,(char*)"Telangana"); //Hyderabad
	break;
case 7:fillellipse(354,399,3,3);outtextxy(358,399,(char*)"Tamil Nadu");   //Chennai
	break;
case 8:fillellipse(372,171,3,3);outtextxy(376,161,(char*)"Uttar Pradesh");   //Lucknow
	break;
case 9:fillellipse(479,225,3,3);outtextxy(483,215,(char*)"West Bengal");    //Kolkata
	break;
case 10:fillellipse(550,160,3,3);outtextxy(554,150,(char*)"Assam");  //Guwahati
	break;
}
}
...
{   settextstyle(-1,0,1);
    setcolor(LIGHTBLUE);
        ...
        {
        if(option==1)button(460,30,530,50,5,LIGHTBLUE,BLACK);
        else button(460,30,530,50,5,LIGHTBLUE,LIGHTBLUE);
        if(option==1){setbkcolor(BLACK);setcolor(LIGHTCYAN);}
        else         {setbkcolor(LIGHTBLUE);setcolor(WHITE);}
        outtextxy(473,32,(char*)"LOG IN");


        if(option==2)button(540,30,610,50,5,LIGHTBLUE,BLACK);
        else button(540,30,610,50,5,LIGHTBLUE,LIGHTBLUE);
        if(option==2){setbkcolor(BLACK);setcolor(LIGHTCYAN);}
        else         {setbkcolor(LIGHTBLUE);setcolor(WHITE);}
        ...
        if(option==3)button(319,309,390,330,5,LIGHTBLUE,BLACK);
        else button(319,309,390,330,5,LIGHTBLUE,LIGHTBLUE);
        if(option==3){setbkcolor(BLACK);setcolor(LIGHTCYAN);}
        else         {setbkcolor(LIGHTBLUE);setcolor(WHITE);}
        outtextxy(329,311,(char*)"SUMBIT");

        if(option==4)button(399,309,445,330,5,LIGHTBLUE,BLACK);
        else button(399,309,445,330,5,LIGHTBLUE,LIGHTBLUE);
        if(option==4){setbkcolor(BLACK);setcolor(LIGHTCYAN);}
        else         {setbkcolor(LIGHTBLUE);setcolor(WHITE);}
...
int program::main_scr_buttons(string &pincode)
{
	...
	switch(getch())
	{
	    case 72:   if(logged)flag=3;
                        else flag=1;
                            main_scr_gfx(flag,logged_user.username);break;
	    case 80:    flag=3;main_scr_gfx(flag,logged_user.username);break;
	    case 75 :if(flag==2||flag==1)flag=1;
                    else if(flag==3||flag==4)flag=3;main_scr_gfx(flag,logged_user.username);break;
	    case 77 :if(flag==1)flag=2;
                    else if(flag==3)flag=4;main_scr_gfx(flag,logged_user.username);break;
	    case 13 :if(flag==3&&pincode.size()==6)return flag;
                    if(flag!=3)return flag; break;
	    case 27 :if(logged)
                    {
                        logged_user=user();
                        logged=false;
	    ...
	    case 48:if(pincode.size()<6)
                    {pincode.append("0");
                        flag=3;main_scr_gfx(flag,logged_user.username);
                    }break;
        case 49:if(pincode.size()<6)
                    {pincode.append("1");
                        flag=3;main_scr_gfx(flag,logged_user.username);
                    }break;
        case 50:if(pincode.size()<6)
                    {pincode.append("2");
                        flag=3;main_scr_gfx(flag,logged_user.username);
                    }break;
        case 51:if(pincode.size()<6)
                    {pincode.append("3");
                        flag=3;main_scr_gfx(flag,logged_user.username);
                    }break;
	    case 52:if(pincode.size()<6)
                    {pincode.append("4");
                        flag=3;main_scr_gfx(flag,logged_user.username);
                    }break;
        case 53:if(pincode.size()<6)
                    {pincode.append("5");
                        flag=3;main_scr_gfx(flag,logged_user.username);
                    }break;
        case 54:if(pincode.size()<6)
                    {pincode.append("6");
                        flag=3;main_scr_gfx(flag,logged_user.username);
                    }break;
        case 55:if(pincode.size()<6)
                    {pincode.append("7");
                        flag=3;main_scr_gfx(flag,logged_user.username);
                    }break;
        case 56:if(pincode.size()<6)
                    {pincode.append("8");
                        flag=3;main_scr_gfx(flag,logged_user.username);
                    }break;
        case 57:if(pincode.size()<6)
                    {pincode.append("9");
                        flag=3;main_scr_gfx(flag,logged_user.username);
                    }break;
        case 8 :if(pincode.size()>0)pincode.pop_back();break;
	    default: continue;
	};
...
{
    setcolor(f_color); setfillstyle(1,f_color);
...
void program::logo_name()
{
    ...
    setbkcolor(LIGHTCYAN);
    settextstyle(10,0,4);
    outtextxy(midx-185,midy-50,(char*)"COVID VACCINE LOCATOR");
}
void program::filter_list(vector<centers> &c_list,vector<bool> filter)
{
    if(filter==(vector<bool>){false,false,false,false}) return;
...
void program::center_scr_gfx(vector<centers>c_list,string district,vector<bool> filter,vector<bool> p_filter,int flag=1)
{
    ...
    if(filter[0])
        {button(110,102,200,123,5,WHITE,BLUE);setbkcolor(BLUE);}
    else
        {button(110,102,200,123,5,BLUE,WHITE);setbkcolor(WHITE);}
    outtextxy(125,105,(char*)"18-44 Only");
    if(filter[1])
        {button(220,102,310,123,5,WHITE,BLUE);setbkcolor(BLUE);}
    else
        {button(220,102,310,123,5,BLUE,WHITE);setbkcolor(WHITE);}
    outtextxy(230,105,(char*)"45 & Above");
    if(filter[2])
        {button(330,102,420,123,5,WHITE,BLUE);setbkcolor(BLUE);}
    else
        {button(330,102,420,123,5,BLUE,WHITE);setbkcolor(WHITE);}
    outtextxy(343,105,(char*)"Covishield");
    if(filter[3])
        {button(440,102,515,123,5,WHITE,BLUE);setbkcolor(BLUE);}
    else
        {button(440,102,515,123,5,BLUE,WHITE);setbkcolor(WHITE);}
    outtextxy(450,105,(char*)" Covaxin ");
    setbkcolor(BLACK);
    settextstyle(8,0,1);
    if(flag==1) button(midx-151,34,midx+181,66,5,BLUE,LIGHTRED);
    else        button(midx-151,34,midx+181,66,5,BLUE,BLACK);
    button(midx-150,35,midx+180,65,5,BLUE,BLACK);

    if(flag==3) button(44,139,getmaxx()-44,getmaxy()-29,10,BLUE,GREEN);
    else button(44,139,getmaxx()-44,getmaxy()-29,10,BLUE,LIGHTGREEN);
    button(45,140,getmaxx()-45,getmaxy()-30,10,BLUE,LIGHTGREEN);
    setbkcolor(LIGHTGREEN);
    setcolor(BLACK);
    if(c_list.size()==0)
        {outtextxy(midx-140,270,(char*)"No Slots Available Right Now"); getch();return;}
    int size=c_list.size();
    int index=0,y=150;
    if(flag>3) index=flag-4;
    if(!district.empty())
    {
        setcolor(YELLOW);
        setbkcolor(BLACK);
    ...
    for(int i=index;i<=index+4&&i<size&&(y<420);i++)
    {
        setbkcolor(WHITE);
        settextstyle(9,0,1);
        if(i==index)    button(49,y-1,getmaxx()-49,y+61,5,BLACK,BLACK);
        else            button(49,y-1,getmaxx()-49,y+61,5,BLACK,WHITE);
        ...
        outtextxy(getmaxx()-180,y+42,(char*)"Review :");
            for(int k=1;k<=5;k++)
            {
                ...
                else {setcolor(BLACK); setfillstyle(1,WHITE);}
...
void program::show_center_details_gfx(centers &c,int flag=1)
{
    ...
    int arr[]={getmaxx()-70,19,getmaxx()-50,19,getmaxx()-50,60,getmaxx()-60,50,getmaxx()-70,60,getmaxx()-70,19};
       if(logged_user.is_bookmarked(c.c_name)){setcolor(DARKGRAY);setfillstyle(1,BLACK);}
       else {setcolor(BLACK);setfillstyle(1,WHITE);}
       if(flag==1)setcolor(LIGHTRED);
       fillpoly(5,arr);
       }
    setcolor(BLACK);
    setbkcolor(LIGHTCYAN);
    settextstyle(-1,0,1);
    if(flag==2)line(150,134,getmaxx()-120,134);
    ...
    outtextxy(80,182,(char*)"Covishield");
    button(60,220,160,260,10,BLACK,WHITE);
    outtextxy(80,232,(char*)"  Covaxin ");
    int x=180;
    for(auto i=c.slot.begin();i!=c.slot.end();i++)
    {   int y=0;
        setbkcolor(BLUE);settextstyle(11,0,1);
        button(x,147,x+72,168,5,WHITE,BLUE);
        outtextxy(x+3,150,(char*)i->date.c_str());
        setcolor(BLACK);setbkcolor(LIGHTGREEN);
        settextstyle(3,0,1);
        if(i->vaccine=="Covaxin")y=50;
        outtextxy(x+22,178+y,(char*)to_string(i->avail_dose).c_str());
        settextstyle(2,0,4);
        outtextxy(x+19,198+y,(char*)("Dose "+to_string(i->dose+1)).c_str());
        x+=90;
    }
    if(flag>=3)button(44,midy+39,getmaxx()-44,getmaxy()-29,10,BLUE,GREEN);
    else button(44,midy+39,getmaxx()-44,getmaxy()-29,10,BLUE,LIGHTGREEN);
    ...
    for(int i=index;i<=index+5&&i<c.review.size()&&(y<420);i++)
    {
        setbkcolor(WHITE);
        settextstyle(-1,0,1);
        if(flag>=3&&i==index)    button(49,y-1,getmaxx()-49,y+21,5,BLACK,BLACK);
        else            button(49,y-1,getmaxx()-49,y+21,5,BLACK,WHITE);
...
void program::show_center_details(centers &c)
{
    ...
    switch(ch)
    {
	    ...
	    case 75:    if(flag==2&&stars>1)stars--;
                        break;
	    case 77:    if(flag==2&&stars<5)stars++;
                        break;
	    case 13:    if(flag==1)
                    {   if(logged)
                            {   if(logged_user.is_bookmarked(c.c_name))
                                    ...
                                    else {logged_user.bookmarked.push_back(c.c_name);}
                            }
                        else {notification("You need to login in-order to bookmark.");
                    ...
                    review="";stars=1;
                        ...
                        break;
	    case 27:    return ;
	    case 9 :    flag=flag%3+1;show_center_details_gfx(c,flag);break;
        case 8 :    if(review.size()>0)review.pop_back();break;
	    default:    if((isalnum(ch)||ch==32)&&review.size()<40&&ch!=7)
                        {flag=2;review+=ch;}
...
void program::show_centers_by_pin(string &pincode)
{
    ...
    string district="";bool found=false;
    int flag=2,f_flag=0,index=1;
    if(p.pin!="")
    {
        center_at_pin=p.center;
        found=true; district=p.district;
    }
	...
	switch(getch())
	{
	    case 72:    if(flag==3)
                        {   if(index>1)
                                index=index-1;
                            center_scr_gfx(center_at_pin,district,filter,filter,flag+index);break;
                        }
	    case 80:    if(flag==3)
                        {
	    ...
	    case  9:    {flag=flag%3+1;center_scr_gfx(center_at_pin,district,filter,filter,flag);break;}
	    case 75:   if(flag==2)
                    {
                        f_flag=(f_flag<=1)?4:(f_flag-1);
                        temp[f_flag-1]=true;
                        center_scr_gfx(center_at_pin,district,temp,filter,flag);break;
                    }
                    else continue;
	    case 77 :if(flag==2)
                    {
	    ...
	    case 13 :   if(flag==1) show_centers_by_pin(pincode);
	    ...
	    case 48:if(pincode.size()<6)
                    {pincode.append("0");
                        flag=1;center_scr_gfx(center_at_pin,district,filter,filter,flag);
                    }break;
        case 49:if(pincode.size()<6)
                    {pincode.append("1");
                        flag=1;center_scr_gfx(center_at_pin,district,filter,filter,flag);
        ...
        case 8 :if(pincode.size()>0)pincode.pop_back(); flag=1;break;
	    default: continue;
	};
	//notification();
    }
...
void program::choose_district_gfx(states s,int flag=0)
{
    ...
    setbkcolor(LIGHTCYAN); setcolor(BLACK);
    outtextxy(midx-50,80,(char*)"Select District :");
    int x=100,y=150,j=0,t=120;
    for(auto i= s.district.begin();i!=s.district.end();i++,j++)
    {
        settextstyle(-1,0,1);
        button(x,y,x+90,y+25,5,BLUE,WHITE);setbkcolor(WHITE);
        if(j==flag)
        {button(x+1,y+1,x+90,y+25,5,WHITE,BLUE);setbkcolor(BLUE);}
    else
        {button(x+1,y+1,x+90,y+25,5,BLUE,WHITE);setbkcolor(WHITE);}
...
{
    vector<centers>center_at_pin;
    string district=d.district_name;bool found=false;
    ...
    for(auto i=d.pin.begin();i!=d.pin.end();i++)
        ...
        if(center_at_pin.size()>0)found=true;
    vector<bool> filter={false,false,false,false};
	...
	switch(getch())
	{
	    ...
	    case 80:
                    ...
                    break;
	    case  9:    {flag=flag%2+1;center_scr_gfx(center_at_pin,district,filter,filter,flag+1);break;}
	    case 75:   if(flag==1)
                    {
                        f_flag=(f_flag<=1)?4:(f_flag-1);
                        temp[f_flag-1]=true;
                        center_scr_gfx(center_at_pin,district,temp,filter,flag+1);break;
                    }
                    break;
	    case 77 :if(flag==1)
                    {
...
void program::show_centers_by_map()
{
    ...
    while(1)
    {
        ...
        {
        case 72:
        case 75:flag=(flag<=0)?n-1:(flag-1);  break;
        case 80:
        case 77:flag=(flag+1)%n;              break;
        case 27: return;
        case 13:
...
void program::main_scr()
{
    ...
    switch(button)
    {
    case -1:    status=false; return;
    case  1:    login_scr();break;
    case  2:    signup_scr();break;
    case  3:    show_centers_by_pin(pincode); break;
    case  4:    show_centers_by_map();break;
    default:break;
    };
    }
...
void program::login_scr_gfx(string u="",string p="",int flag=1)
{
    ...
    border_gfx();
    string temp="";
    for(int i=0;i<p.size();i++)temp+="* ";if(temp.size()==0)temp="Password";
    if(u.size()==0)u="Username";
    ...
    if(flag==1)button(379,199,591,231,10,DARKGRAY,DARKGRAY);
    else button(379,199,591,231,10,DARKGRAY,WHITE);
    button(380,200,590,230,10,DARKGRAY,WHITE);
    {
        int arr[]={387,214,394,214,394,220,387,220,387,214};
        setfillstyle(1,BLACK); setcolor(BLACK); fillpoly(4,arr); fillellipse(391,210,3,3);
    }
    setcolor(DARKGRAY);
    outtextxy(400,207,(char*)u.c_str());
    if(flag==2)button(379,244,591,276,10,DARKGRAY,DARKGRAY);
    else button(379,244,591,276,10,DARKGRAY,WHITE);
    button(380,245,590,275,10,DARKGRAY,WHITE);
    {
        int arr[]={387,259,394,259,394,265,387,265,387,259};
        setfillstyle(1,BLACK); setcolor(BLACK); fillpoly(4,arr); arc(391,259,0,180,3);
    }
    setcolor(DARKGRAY);
    outtextxy(400,252,(char*)temp.c_str());
    if(flag==3)button(534,289,601,311,5,BLACK,DARKGRAY);
    else button(534,289,601,311,5,BLACK,WHITE);
    button(535,290,600,310,5,BLACK,WHITE);
    setbkcolor(WHITE);settextstyle(-1,0,1);
    outtextxy(547,292,(char*)"LOGIN");
    setbkcolor(LIGHTCYAN);setcolor(DARKGRAY);
    outtextxy(385,422,(char*)"Don't have an account?");
    setcolor(BLACK);
    if(flag==4)setcolor(LIGHTRED);
    outtextxy(530,422,(char*)"Sign Up");
    line(530,440,580,440);
    if(flag==4)line(530,442,580,442);
}
void program::login_scr()
{
    ...
    switch(ch)
    {
        case 72:    flag=(flag<=1)?4:(flag-1);
                    login_scr_gfx(username,password,flag);
                    break;
        case 80:    flag=flag%4+1;
                    login_scr_gfx(username,password,flag);
                    break;
	    case 75:    break;
	    case 77:    break;
	    case 13:    if(flag==4)
                    ...
                    {
                        if(username.size()==0){error="Empty User-name . Try again ";notification(error);}
                        else flag++;
                    }
                    else if(flag==2)
                    {
                        if(password.size()==0){error="Please Enter Password . Try again ";notification(error);}
                        else flag++;
                    }
                    if(flag==3)
                    {
                        ...
                        for(auto i=user_list.begin();i!=user_list.end()&&(exist==false);i++)
                        {
                            if(i->username==username)
                            {
                                ...
                                else {error="Incorrect Password. Try again.";notification(error);}
                        ...
                        else {main_scr();return;}
                    }
                    login_scr_gfx(username,password,flag);
                    break;
	    case 27:    return ;
	    case 9 :    flag=flag%4+1;login_scr_gfx(username,password,flag);break;
        case 8 :    if(flag==1&&username.size()>0) username.pop_back();
                        else if(flag==2&&password.size()>0) password.pop_back();
                    login_scr_gfx(username,password,flag);
                    break;
	    default:    if((isalnum(ch)||ch==32)&&ch!=7&&flag<3)
                        {
                            if(flag==1&&username.size()<=15) username+=ch;
                                else if(flag==2&&password.size()<=15) password+=ch;
                            login_scr_gfx(username,password,flag);
                        }
...
void program::signup_scr_gfx(string u="",string p="",string r="",string m="",int flag=1)
{
    ...
    settextstyle(-1,0,1);
    setcolor(BLACK);setbkcolor(WHITE);
    if(flag==1)button(49,154,271,186,10,DARKGRAY,DARKGRAY);
    else button(49,154,271,186,10,DARKGRAY,WHITE);
    button(50,155,270,185,10,DARKGRAY,WHITE);
    {
        int arr[]={57,169,64,169,64,175,57,175,57,169};
        setfillstyle(1,BLACK); setcolor(BLACK); fillpoly(4,arr); fillellipse(61,165,3,3);
    }
    setcolor(DARKGRAY);
    outtextxy(70,162,(char*)u.c_str());
    if(flag==2) button(49,199,271,231,10,DARKGRAY,DARKGRAY);
        else button(49,199,271,231,10,DARKGRAY,WHITE);
    button(50,200,270,230,10,DARKGRAY,WHITE);
    {
        int arr[]={57,214,64,214,64,220,57,220,57,214};
        setfillstyle(1,BLACK); setcolor(BLACK); fillpoly(4,arr); arc(61,214,0,180,3);
    }
    setcolor(DARKGRAY);
    for(int i=0;i<p.size();i++)temp+="* ";
    if(temp.size()==0)temp="Password";
    outtextxy(70,207,(char*)temp.c_str());
    if(flag==3) button(49,244,271,276,10,DARKGRAY,DARKGRAY);
        else button(49,244,271,276,10,DARKGRAY,WHITE);
    button(50,245,270,275,10,DARKGRAY,WHITE);
    {
        int arr[]={57,259,64,259,64,265,57,265,57,259};
        setfillstyle(1,BLACK); setcolor(BLACK); fillpoly(4,arr); arc(61,259,0,180,3);
    }
    setcolor(DARKGRAY);
    temp.clear();
    for(int i=0;i<r.size();i++)temp+="* ";
    if(temp.size()==0)temp="Re-Password";
    outtextxy(70,252,(char*)temp.c_str());
    if(flag==4) button(49,289,271,321,10,DARKGRAY,DARKGRAY);
        else button(49,289,271,321,10,DARKGRAY,WHITE);
    button(50,290,270,320,10,DARKGRAY,WHITE);
    {
        int arr[]={64,303,58,308,59,309,64,302};
        setfillstyle(1,BLACK); setcolor(BLACK);
        fillpoly(4,arr);fillellipse(63,302,2,3);fillellipse(57,309,2,3);
    }
    setcolor(DARKGRAY);
    outtextxy(70,297,(char*)m.c_str());
    if(flag==5) button(204,334,271,356,5,BLACK,DARKGRAY);
        else button(204,334,271,356,5,BLACK,WHITE);
    button(205,335,270,355,5,BLACK,WHITE);
    setbkcolor(WHITE);settextstyle(-1,0,1);
    outtextxy(212,337,(char*)"SIGN UP");
    setcolor(DARKGRAY);setbkcolor(LIGHTCYAN);
    outtextxy(55,422,(char*)"Already have an account?");
    setcolor(BLACK);
    setcolor(BLACK);
    if(flag==6){setcolor(LIGHTRED);line(218,442,258,442);}
...
void program::signup_scr()
{
    ...
    switch(ch)
    {
	    ...
	    case 13:    if(flag==6)
                    ...
                    {
                        if(repass.size()==0){error="Please Re-Enter Password . Try again ";notification(error);}
                        else flag++;
                    }
                    else if(flag==4)
                    {
                        if(mobile.size()==0){error="Please Enter Valid Mobile Number.";notification(error);}
                        else flag++;
                    }
                    if(flag==5)
                    {   bool already= false;
                        if(password!=repass){error="Password Don't Match. Try Again.";notification(error);}
                        else
                        {
                        for(auto i=user_list.begin();i!=user_list.end();i++)
                        {
                            if(i->username==username) {already=true;} break;
                        }
                        if(already){error="You Already Have an account try logging in.";flag=6;notification(error);}
                            else    {
                    ...
                    signup_scr_gfx(username,password,repass,mobile,flag);
                        break;
	    case 27:    return ;
	    case 9 :    flag=flag%6+1;signup_scr_gfx(username,password,repass,mobile,flag);break;
        case 8 :    if(flag==1&&username.size()>0) username.pop_back();
                        else if(flag==2&&password.size()>0) password.pop_back();
                        else if(flag==3&&repass.size()>0) repass.pop_back();
                        else if(flag==4&&mobile.size()>0) mobile.pop_back();
                    signup_scr_gfx(username,password,repass,mobile,flag);
                    break;
	    default:    if((isalnum(ch)||ch==32)&&ch!=7&&flag<=4)
                        {   if((flag==4)&&(isdigit(ch))&&mobile.size()<10)mobile+=ch;
                            if(flag==1&&username.size()<=15) username+=ch;
                                else if(flag==2&&password.size()<=15) password+=ch;
                                    else if(flag==3&&repass.size()<=15)repass+=ch;
                            signup_scr_gfx(username,password,repass,mobile,flag);
                        }
...
int main()
{
        ...
        midx=getmaxx()/2;
        midy=getmaxy()/2;
        program covid_locator=program("run");
        getch();
    return 0;
}
